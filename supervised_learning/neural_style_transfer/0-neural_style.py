...
class NST:
    ...
    def __init__(self, style_image, content_image, alpha=1e4, beta=1):
        ...
        """
        if (not isinstance(style_image, K.backend.floatx()) or
           style_image.ndim != 3 or style_image.shape[2] != 3):
            raise TypeError("style_image must be a numpy.ndarray "
                            "of shape (h, w, 3)")
        if (not isinstance(content_image, K.backend.floatx()) or
    ...
    def scale_image(self, image):
        ...
        if h > w:
            ...
            new_w = 512
            new_h = int(h * (512 / w))
        resized_image = K.backend.resize_images(
            image, new_h / h, new_w / w, "channels_last",
